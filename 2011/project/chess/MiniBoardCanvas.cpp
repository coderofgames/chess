...
BEGIN_EVENT_TABLE(MiniBoardCanvas, wxScrolledWindow)
    EVT_PAINT  (MiniBoardCanvas::OnPaint)
	EVT_IDLE(MiniBoardCanvas::OnIdle)
    EVT_MOTION (MiniBoardCanvas::OnMouseMove)
    EVT_SIZE(MiniBoardCanvas::OnSize)

   // EVT_MOUSE_EVENTS(BoardCanvas::OnMouse)
	EVT_LEFT_DOWN(MiniBoardCanvas::mouseDown)
	EVT_LEFT_UP(MiniBoardCanvas::mouseReleased)
	EVT_MOUSEWHEEL(MiniBoardCanvas::mouseWheelMoved)
    EVT_ERASE_BACKGROUND(MiniBoardCanvas::OnEraseBackground)
END_EVENT_TABLE()

MiniBoardCanvas::MiniBoardCanvas(MyFrame *parent, wxWindow *winParent)
                             ...
                             wxVSCROLL | wxFULL_REPAINT_ON_RESIZE |wxWANTS_CHARS )
{
	...
	only_show_pawns = false;


    // connect event handlers for the blue input window
    Connect(wxEVT_KEY_DOWN, wxKeyEventHandler(MiniBoardCanvas::OnKeyDown),
                        NULL, this);
    Connect(wxEVT_KEY_UP, wxKeyEventHandler(MiniBoardCanvas::OnKeyUp),
                        NULL, this);
    //Connect(wxEVT_CHAR, wxKeyEventHandler(MiniBoardCanvas::OnChar),
     //                   NULL, this);
}

MiniBoardCanvas::~MiniBoardCanvas()
{

//	delete chess_board;
}

bool MiniBoardCanvas::PickSquare( double x, double y, int START_X, int END_X, 
...
//			selState = SELECTED_SQUARE;
	...
	if( only_show_pawns )
	{
		if(x >= addPieces_x && x < addPieces_x +squareSize*1)
		{
		...
		}else selState = SelectionState::SEL_NONE;
	}
	else
	{
...
void MiniBoardCanvas::OnEraseBackground(wxEraseEvent& WXUNUSED(evt))
{
}

void MiniBoardCanvas::InitGraphics()
{
}


void MiniBoardCanvas::ResetBoard()
{
	sz = GetClientSize();
...
void MiniBoardCanvas::OnPaint(wxPaintEvent &event)
{
    wxPaintDC pdc(this);
...
//	DrawGraphics();
	...
	addPieces_x = base2_x +border+squareSize*1.5;
	addPieces_y = base2_y + 2*border+squareSize*8.5;

	whiteBorderBox.AddRectangle( addPieces_x, addPieces_y, squareSize*5, squareSize);
	...
	gc->PushState(); // save current translation/scale/other state
	{
		...
		gc->SetBrush(wxBrush(wxColour(150,  150,  255, 128)));// 128 == half transparent
 	...
 	for( double x = base2_x+border ; x < base2_x+border+ squareSize*8; x+= squareSize )
	{
		...
		for( double y = base2_y+border; y < base2_y+border+ squareSize*8; y+=squareSize)
		{
			...
			{
				if( (only_show_pawns && (((pcID-1) == 0) || ((pcID-1) == 6))) || (!only_show_pawns) )
				{
...
//#endif			
	...
	gc->PushState();
			{
				gc->SetPen(wxPen("red", 1));
				//gc->SetBrush(wxBrush(wxColour(150,  150,  255, 0)));// 128 == half transparent
				gc->DrawPath(blackBorderBox);
			}
	gc->PopState();
...
void MiniBoardCanvas::OnIdle(wxIdleEvent &event)
{
	wxPaintEvent evt;
	OnPaint(evt);
...
void MiniBoardCanvas::mouseDown(wxMouseEvent& event)
{
	 wxClientDC dc(this);
    PrepareDC(dc);
    m_parent->PrepareDC(dc);
...
void MiniBoardCanvas::CalculateSizeParameters()
{
...
void MiniBoardCanvas::OnSize(wxSizeEvent& event)
{
	bResize = true;
	sz = GetClientSize();
	CalculateSizeParameters();
...
void MiniBoardCanvas::OnMouseMove(wxMouseEvent &event)
{
/*#if wxUSE_STATUSBAR
    wxClientDC dc(this);
    PrepareDC(dc);
...
void MiniBoardCanvas::mouseReleased(wxMouseEvent& event)
{
	 wxClientDC dc(this);
    PrepareDC(dc);
    m_owner->PrepareDC(dc);
...
void MiniBoardCanvas::LogEvent(const wxString& name, wxKeyEvent& event)
{
