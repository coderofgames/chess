    ...
    file_mapping_handle_(INVALID_HANDLE_VALUE)
{
	...
	wchar_t *pwcs = new wchar_t[n+1];
	mbstowcs(pwcs, pathname, n);
	pwcs[n]= L'\0';
    file_handle_ = ::CreateFile(pwcs, GENERIC_READ,
        FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	delete [] pwcs;
    if (file_handle_ == INVALID_HANDLE_VALUE) 
	{
		cout<< "Invalid Handle Error" << endl;
			return;
	}
	...
	else wprintf(L"Format message failed with 0x%x\n", GetLastError());
#endif
}
    ...
    file_mapping_handle_(INVALID_HANDLE_VALUE)
{
	...
	mbstowcs(pwcs, pathname, n);
	pwcs[n]= L'\0';
    file_handle_ = ::CreateFile(pwcs, GENERIC_READ | GENERIC_WRITE,
