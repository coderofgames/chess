...
BEGIN_EVENT_TABLE(TreeStatsCanvas, wxScrolledWindow)
//	EVT_CLOSE(PositionSetupFrame::OnClose)
	EVT_PAINT  (TreeStatsCanvas::OnPaint)
	EVT_IDLE (TreeStatsCanvas::OnIdle)
	EVT_LEFT_DOWN (TreeStatsCanvas::OnMouseLeftClick)
	
END_EVENT_TABLE()

TreeStatsCanvas::TreeStatsCanvas(MyFrame *parent)
...
TreeStatsCanvas::~TreeStatsCanvas()
{
}



void TreeStatsCanvas::OnPaint(wxPaintEvent &event)
{
	bool m_useContext = true;
    wxPaintDC pdc(this);
	
	static bool setScrollbar = false;
//	m_frame->ClassifyECO();
...
//	gc->DrawBitmap(m_bmp, 0,0, 554, 534 ); 
	...
	{
		setScrollbar = true;
		SetScrollbars( 10, 10, rectStart + 50, sz.GetHeight()/10 + 200);
	}
	...
	if( m_bUsingExtraStats == false )
	{
		for( int i = 0; i < this->vecResults.size(); i++ )
		{
			...
			/*gc->DrawText(wxString::Format("%f, %f, %f, %d", 
				vecResults[i]->GetWhiteWinsPercentage(),
				vecResults[i]->GetBlackWinsPercentage(),
				vecResults[i]->GetDrawPercentage(),
				vecResults[i]->GetTotal()),wSan+20,i*20,0);
			*/
...
struct CompareResults {
	bool operator ()(PGN::ResultCache *lhs, PGN::ResultCache *rhs) { return lhs->GetTotal() > rhs->GetTotal(); }
};

struct CompareResultsExtraStats {
	bool operator ()(PGN::ResultCacheExtraStats *lhs, PGN::ResultCacheExtraStats *rhs) { return lhs->GetTotal() > rhs->GetTotal(); }
};

void TreeStatsCanvas::SortResultCaches()
{
	if( !m_bUsingExtraStats )
	{
		sort(this->vecResults.begin(), this->vecResults.end(), CompareResults());
	}
	else
	{
		sort(this->vecResultsExtraStats.begin(), this->vecResultsExtraStats.end(), CompareResultsExtraStats());
	}
}


void TreeStatsCanvas::OnIdle(wxIdleEvent &event)
{
	event.Skip();
}


void TreeStatsCanvas::OnMouseLeftClick(wxMouseEvent &event)
{
