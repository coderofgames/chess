...
bool HashCompare(uint64 key, uint64 flag)
{
	return (key & flag) == flag;
}

uint64 KeySet(uint64 key, uint64 flag)
{
	key ^= flag;
	return key;
}
/*
uint64 PieceMove( ChessBoard* board, uint64 key,  int x1, int y1, int x2, int y2 )
{
	int pcDest = board->board[x2][y2];
	if( pcDest != 0 )
	{
		char pc_code = board->PieceCodeToChar( pcDest );
		uint64 p_enc=strchr(piece_names,pc_code)-piece_names;
		key^=RandomPiece[64*p_enc+8*y2+x2];
	}

	int pcSrc = board->board[x1][y1];
	uint64 p_enc2=strchr(piece_names,pcSrc)-piece_names;
	key^=RandomPiece[64*p_enc2+8*y2+x2];
