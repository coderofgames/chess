...
bool MyPipePanel::CheckTime()
{
	if( engineColor == WHITE )
	{
		...
		else 
		{
			...
			if( pParent->board_canvas->GetBoard()->DoesBlackHaveAnyPieces() == false )
			{
	...
	else
	{
		...
		else
		{
			...
			if( pParent->board_canvas->GetBoard()->DoesWhiteHaveAnyPieces() == false )
			{
				pParent->currentGame->result = "1/2-1/2";
				if( pParent->optionsFrame->m_GameOptions.showMessageBoxAtGameEnd == true )
				{
...
void MyPipePanel::WaitBestMove(char* buf, wxTextCtrl *text)
{
	...
	if( pParent->board_canvas->GetBoard()->FiftyMoveTest() )
	{
		DoFinishGame();
		return;
	}
	...
	for( int i = 0; i < tokens.size(); i++ )
	{
		...
		if( tokens[i] == "pv" )//&& timeMode > HYPER_BULLET )
		{
			int j = i+1;
	...
	if( bestmove != "" )
	{
			...
			case CHECKMATE:
				{
					if( pParent->board_canvas->GetBoard()->mov == 0 || pParent->board_canvas->GetBoard()->mov == 2 )
					{
						wxString gameResultString = "Black Checkmates!";
						if( pParent->optionsFrame->m_GameOptions.showMessageBoxAtGameEnd == true )
						{
					...
					{
						wxString gameResultString = "White Checkmates!";
						if( pParent->optionsFrame->m_GameOptions.showMessageBoxAtGameEnd == true )
						{
							wxMessageBox( gameResultString,"Game Over");
						}
			...
			case STALEMATE:
				{
					if( pParent->board_canvas->GetBoard()->mov == 0 || pParent->board_canvas->GetBoard()->mov == 2 )
					{
						wxString gameResultString = "White Stalemated!";
						if( pParent->optionsFrame->m_GameOptions.showMessageBoxAtGameEnd == true )
						{
							wxMessageBox( gameResultString,"Game Over");
						}
					...
					{
						wxString gameResultString = "Black Stalemated!";
						if( pParent->optionsFrame->m_GameOptions.showMessageBoxAtGameEnd == true )
						{
							wxMessageBox( gameResultString,"Game Over");
						}
			...
			case DRAW_BY_50_MOVE:
				{
					if( pParent->board_canvas->GetBoard()->mov == 0 || pParent->board_canvas->GetBoard()->mov == 2 )
					{
						wxString gameResultString = "Drawn, 50 moves without a pawn move or capture!";
		...
		if( (repetitions >= 3) || (pParent->board_canvas->GetBoard()->fiftyMov >= 50) ||
			(materialDraw))
		{
			goMode = GO_TIME;
			opponent->SetGoMode(GO_TIME);
