...
//	ChessSquare squares[8][8];
	...
	{
		if( target && (target->to.file >=0 && target->to.file < 8) &&
					  (target->to.rank >=0 && target->to.rank < 8))
		{
	...
	string Fen()
	{
			...
			for( int x = 0; x <8; x++ )
			{
				...
				if( board[x][y]!= 0)
				{
					if( blankSquares > 0 )
					{
						...
						//output += "/";
						static char buf3[20];
						sprintf( buf3, "%d", (blankSquares));
						//std::stringstream blanks;
						//blanks << blankSquares;
						output += buf3;//blanks.str();
						blankSquares = 0;
					}
				...
				if( index % 8 == 7 )
				{
					...
					{
						static char buf0[20];
						sprintf( buf0, "%d", (blankSquares));
						//std::stringstream blanks;
						//blanks << blankSquares;
						output += buf0;//blanks.str();
						output += "/";
						blankSquares = 0;
					}
					else
		...
		if( canEmpasse )
		{
			...
			//output += blanks.str();
			static char buf4[20];
			sprintf( buf4, "%d", (empassantTarget.sq.rank + 1));
			output += buf4;
		}
		else
		{
			output += spacer + "-";
			this->empassantTarget.sq.file = 0;
			this->empassantTarget.sq.rank = 0;
		}
		...
		if( mov == 1 && movCount == 0 )
		{
			...
			output+= buf;//movCountInfoStrStr.str();
		}
		else
		{
			static char buf[20];

			sprintf( buf, "%d", (movCount));
			//std::stringstream movCountInfoStrStr;
	...
	void ClearBoard()
	{
		...
		for( unsigned int i = 0; i < blackPcs.size(); i++ )
		{
		...
		for( unsigned int i = 0; i < whitePcs.size(); i++ )
		{
	...
	void GenLegalMovesKnight(MyChessPiece *N)
	{
		if( isPiecePinned(N ))
			return;

		MyBigChessMove moves[8];

		for( int i = 0; i < 8; i++ )
		{
			moves[i].from.file = N->x;
			moves[i].from.rank = N->y;
		}
				...
				moves[i].to.rank > -1 && moves[i].to.rank < 8 )
			{
				if( N->color == WHITE )
				{
						...
						board[moves[i].to.file][moves[i].to.rank] != wKing )
					{
						...
						MyChessMove mv;
						mv.from.file = moves[i].from.file;
						mv.from.rank = moves[i].from.rank;
						mv.to.file = moves[i].to.file;
						mv.to.rank = moves[i].to.rank;
						legal_moves[numLegalMoves++].mv = mv;
					}
				}
				else
				{
	...
	void GenLegalMovesBishop(MyChessPiece *B)
	{
		if( isPiecePinned(B ))
		{
			int fromX = B->PinFromX;
			int fromY = B->PinFromY;
			...
			if( B->color == WHITE )
			{
				...
				{
					/*
					int X1 = (fromX > whitePcs[4]->x) ? whitePcs[4]->x : fromX;
					int X2 = (fromX > whitePcs[4]->x) ? fromX : whitePcs[4]->x;

					int Y1 = fromY > whitePcs[4]->y ? whitePcs[4]->y : fromY;
					int Y2 = fromY > whitePcs[4]->y ? fromX : whitePcs[4]->y;
					*/
					if( fromX > whitePcs[4]->x )
					{
						if( fromY > whitePcs[4]->y )
						{
							for( int x = whitePcs[4]->x + 1, y = whitePcs[4]->y + 1; (x<= fromX)&&(y<= fromY); x++, y++)
							{
								if( (x!= B->x) && (y!=B->y))
								{
									MyChessMove mv;
			...
			else
			{
				...
				{
	/*				int X1 = (fromX > blackPcs[4]->x) ? blackPcs[4]->x : fromX;
					int X2 = (fromX > blackPcs[4]->x) ? fromX : blackPcs[4]->x;

					int Y1 = (fromY > blackPcs[4]->y) ? blackPcs[4]->y : fromY;
					int Y2 = (fromY > blackPcs[4]->y) ? fromX : blackPcs[4]->y;
					...
					legal_moves[numLegalMoves++].mv = mv;
*/
					if( fromX > blackPcs[4]->x )
					{
						if( fromY > blackPcs[4]->y )
						{
							for( int x = blackPcs[4]->x + 1, y = blackPcs[4]->y + 1; (x<= fromX)&&(y<= fromY); x++, y++)
							{
								if( (x!= B->x) && (y!=B->y))
								{
									MyChessMove mv;
		...
		for( int x=x1+1, y=y1+1; (y<8)&&(x<8); x++, y++)
		{
			...
			if( B->color == WHITE )
			{
					...
					board[x][y] != wKing )
				{
					...
					legal_moves[numLegalMoves++].mv = move;
	...
	void GenLegalMovesRook(MyChessPiece *R)
	{
		if( isPiecePinned(R) )
		{
			int fromX = R->PinFromX;
			int fromY = R->PinFromY;
			if( R->color == WHITE )
			{
				if( R->x != whitePcs[4]->x &&
					R->y != whitePcs[4]->y )
				...
				else
				{
					...
					if( R->x == whitePcs[4]->x )
					{
						if( fromY > whitePcs[4]->y )
						{
							for( int y = whitePcs[4]->y + 1; y<= fromY; y++ )
							{
								if( y!= R->y )
								{
									MyChessMove mv;
					...
					else if( R->y == whitePcs[4]->y )
					{
						if( fromX > whitePcs[4]->x )
						{
							for( int x = whitePcs[4]->x + 1; x<= fromX; x++ )
							{
								if( x!= R->x )
								{
									MyChessMove mv;
			...
			else
			{
				...
				else
				{
					if( R->x == blackPcs[4]->x )
					{
						if( fromY > blackPcs[4]->y )
						{
							for( int y = blackPcs[4]->y + 1; y<= fromY; y++ )
							{
								if( y!= R->y )
								{
									MyChessMove mv;
					...
					else if( R->y == blackPcs[4]->y )
					{
						if( fromX > blackPcs[4]->x )
						{
							for( int x = blackPcs[4]->x + 1; x<= fromX; x++ )
							{
								if( x!= R->x )
								{
									MyChessMove mv;
	...
	void GenLegalMovesQueen(MyChessPiece *Q)
	{
		if( isPiecePinned(Q ))
		{
			int fromX = Q->PinFromX;
			int fromY = Q->PinFromY;
	...
	void GenLegalMovesKing(MyChessPiece *K)
	{
		int numSquares = 0;
		MyBigChessSquare king_squares[8];
		...
		if( K->color == WHITE )
		{
			...
			if( !IsSquareDirectlyAttacked(BLACK, K->x, K->y) )
			{
				if( this->canWhiteCastleLong() )
				{
					if( !this->isRankSpanBlocked(K->x, 0, 0) )
					{
						if( !this->IsSquareDirectlyAttacked(BLACK, K->x-1, 0) && 
							!this->IsSquareDirectlyAttacked(BLACK, K->x-2, 0))
						{
							MyChessMove move;
							move.from.file = K->x;
							move.from.rank = K->y;
				...
				{
					if( !this->isRankSpanBlocked(K->x, 7, 0) )
					{
						if( !this->IsSquareDirectlyAttacked(BLACK, K->x+1, 0) &&
							!this->IsSquareDirectlyAttacked(BLACK, K->x+2, 0))
						{
		...
		else
		{
			...
			if( !IsSquareDirectlyAttacked(WHITE, K->x, K->y) )
			{
				if( this->canBlackCastleLong() )
				{
					if( !this->isRankSpanBlocked(K->x, 0, 7) )
					{
						if( !this->IsSquareDirectlyAttacked(WHITE, K->x-1, 7) && 
							!this->IsSquareDirectlyAttacked(WHITE, K->x-2, 7))
						{
							MyChessMove move;
							move.from.file = K->x;
							move.from.rank = K->y;
				...
				{
					if( !this->isRankSpanBlocked(K->x, 7, 7) )
					{
						if( !this->IsSquareDirectlyAttacked(WHITE, K->x+1, 7) && 
							!this->IsSquareDirectlyAttacked(WHITE, K->x+2, 7))
						{
	...
	void GenLegalMovesPawn(MyChessPiece *P)
	{
		if( P->color == WHITE )
		{
			bool pinned = isPiecePinned(P);
			if( P->y == 1 )
			{

				if( CanPawnReachSquare(P, P->x, P->y +2) )
				{
					MyChessMove move2;
					move2.from.file = P->x;
					move2.from.rank = P->y;
					...
					{	
						if( (P->PinFromX == P->x) )
						{
							legal_moves[numLegalMoves++].mv = move2;
						}
					}
					else
					{
			...
			for( int i = 0; i < 3; i++ )
			{
				...
				{
					if( moves[i].to.rank == 7 )
					{

						if( (pinned &&  (P->x != P->PinFromX && P->y != P->PinFromY)) &&
							((P->PinFromX > P->x) && (moves[i].to.file > P->x)||
							(P->PinFromX < P->x) && (moves[i].to.file < P->x)))
						{
							MyChessMove move3;
							move3.from.file = moves[i].from.file;
							move3.from.rank = moves[i].from.rank;
							move3.to.file = moves[i].to.file;
							move3.to.rank = moves[i].to.rank;

							legal_moves[numLegalMoves++].mv = move3;
							move3.promo = 0;
							legal_moves[numLegalMoves++].mv = move3;
							move3.promo = 1;
							legal_moves[numLegalMoves++].mv = move3;
					...
					else
					{
						...
						if( pinned )
						{
							if( (moves[i].to.file == P->x) && (P->x == P->PinFromX))
							{
								legal_moves[numLegalMoves++].mv = move3;
							}
							else if( P->x != P->PinFromX && P->y != P->PinFromY )
							{
								if( (P->PinFromX > P->x) && (moves[i].to.file > P->x) )
								{
									legal_moves[numLegalMoves++].mv = move3;
								}
								else if( (P->PinFromX < P->x) && (moves[i].to.file < P->x) )
								{
									legal_moves[numLegalMoves++].mv = move3;
								}
							}
			...
			if( this->canEmpasse )
			{
				if( P->x == this->empassantTarget.sq.file + 1 || 
					P->x == this->empassantTarget.sq.file - 1 )
				{
					if( P->y == this->empassantTarget.sq.rank - 1 )
					{
						MyBigChessMove movesEP;
						movesEP.from.file = P->x;
						movesEP.from.rank = P->y;
						movesEP.to.file = this->empassantTarget.sq.file;
						movesEP.to.rank = this->empassantTarget.sq.rank;

						MyChessMove move4;
						move4.from.file = movesEP.from.file;
						move4.from.rank =movesEP.from.rank;
						move4.to.file = movesEP.to.file;
						move4.to.rank = movesEP.to.rank;
						if( pinned )
						{

							if( P->x != P->PinFromX && P->y != P->PinFromY )
							{
								if( (P->PinFromX > P->x) && (movesEP.to.file > P->x) )
								{


									legal_moves[numLegalMoves++].mv = move4;
								}
								else if( (P->PinFromX < P->x) && (movesEP.to.file < P->x) )
								{
		...
		if( mov != BLACK )
		{
			for( int i = 0; i < whitePcs.size(); i++ )
			{
				if( !whitePcs[i]->bCaptured )
				{
					...
					{
					case Pawn:
						GenLegalMovesPawn(whitePcs[i]);
						break;
					case Knight:
						GenLegalMovesKnight(whitePcs[i]);
						break;
					case Bishop:
						GenLegalMovesBishop(whitePcs[i]);
						break;
					case Rook:
						GenLegalMovesRook(whitePcs[i]);
						break;
					case Queen:
						GenLegalMovesQueen(whitePcs[i]);
						break;
					};
		...
		else
		{
			for( int i = 0; i < blackPcs.size(); i++ )
			{
				if( !blackPcs[i]->bCaptured )
				{
					...
					{
					case Pawn:
						GenLegalMovesPawn(blackPcs[i]);
						break;
					case Knight:
						GenLegalMovesKnight(blackPcs[i]);
						break;
					case Bishop:
						GenLegalMovesBishop(blackPcs[i]);
						break;
					case Rook:
						GenLegalMovesRook(blackPcs[i]);
						break;
					case Queen:
						GenLegalMovesQueen(blackPcs[i]);
						break;
					};
				...
				else
				{
					if( attackers.size() == 1 )
					{
						...
						if( defensiveMoves.size() == 0 )
						{
							if( numSquares != 0 )
							{
								if( attackers[0]->type == Pawn )
								{
									if( this->canEmpasse == true )
									{
										if( (this->empassantTarget.sq.file == attackers[0]->x) )
										{
											if( this->empassantTarget.sq.rank == attackers[0]->y+1 )
											{
												if( (attackers[0]->x+1 < 8))
												{
													if( board[attackers[0]->x + 1][attackers[0]->y]==wPawn)
													{
														MyChessMove mv;
														mv.from.file = attackers[0]->x+1;
														mv.from.rank = attackers[0]->y;
														mv.to.file = this->empassantTarget.sq.file;
														mv.to.rank = this->empassantTarget.sq.rank;
														legal_moves[numLegalMoves++].mv = mv;//defensiveMoves[i];
														return CHECK;
													}
												}
												if( (attackers[0]->x-1 >= 0) )
												{
													if( board[attackers[0]->x - 1][attackers[0]->y]==wPawn)
													{
														MyChessMove mv;
						...
						else
						{
							...
							{
								legal_moves[numLegalMoves++].mv = defensiveMoves[i];
									...
									if( this->canEmpasse == true )
									{
										if( (this->empassantTarget.sq.file == attackers[0]->x) )
										{
											if( this->empassantTarget.sq.rank == attackers[0]->y-1 )
											{
												if( (attackers[0]->x+1 < 8))
												{
													if( board[attackers[0]->x + 1][attackers[0]->y]==bPawn)
													{
														MyChessMove mv;
														mv.from.file = attackers[0]->x+1;
														mv.from.rank = attackers[0]->y;
	...
	bool IsMovePromotingPawn(MyChessMove *mv)
	{
		if( (mv->from.file >=0 && mv->from.file < 8) &&
			(mv->from.rank >=0 && mv->from.rank < 8) &&
			(mv->to.file >=0 && mv->to.file < 8) &&
			(mv->to.rank >=0 && mv->to.rank < 8) )
		{
			if( board[mv->from.file][mv->from.rank] == wPawn )
			{
				if( (mv->to.rank > mv->from.rank) )
				{
					if( mv->to.rank == 7 )
						return true;
				}
			...
			if( board[mv->from.file][mv->from.rank] == bPawn )
			{
				if( (mv->to.rank < mv->from.rank) )
				{
					if( mv->to.rank == 0 )
				...
				if( K->color == WHITE )
				{
					if( !whitePcs[i]->bCaptured )
					{
						if( !isPiecePinned(whitePcs[i]))
						{
							...
							if( K->y < fromY )
							{
								for( int y = K->y+1; y <= fromY; y++ )
								{
									if( whitePcs[i]->type == Pawn )
									{
										if( CanPawnReachSquare(whitePcs[i], K->x, y ))
										{
											mv.to.file = K->x;
											mv.to.rank = y;
									...
									else if( CanPieceReachSquare(whitePcs[i], K->x, y ))
									{
										mv.to.file = K->x;
										mv.to.rank = y;
										legalMoves->push_back(mv);
						...
						if( !isPiecePinned(blackPcs[i]))
						{
							...
							if( K->y < fromY )
							{
								for( int y = K->y+1; y <= fromY; y++ )
								{
									...
									{
										if( CanPawnReachSquare(blackPcs[i], K->x, y ))
										{
											mv.to.file = K->x;
											mv.to.rank = y;
											legalMoves->push_back(mv);
									...
									else if( CanPieceReachSquare(blackPcs[i], K->x, y ))
									{
										mv.to.file = K->x;
										mv.to.rank = y;
										legalMoves->push_back(mv);
				...
				if( K->color == WHITE )
				{
					if( !whitePcs[i]->bCaptured )
					{
						if( !isPiecePinned(whitePcs[i]))
						{
							...
							if( K->x < fromX )
							{
								for( int x = K->x+1; x <= fromX; x++ )
								{
									if( whitePcs[i]->type == Pawn )
									{
										if( CanPawnReachSquare(whitePcs[i], x, K->y ))
										{
											mv.to.file = x;
											mv.to.rank = K->y;
											legalMoves->push_back(mv);
									...
									else if( CanPieceReachSquare(whitePcs[i], x, K->y ))
									{
										mv.to.file = x;
										mv.to.rank = K->y;
										legalMoves->push_back(mv);
						...
						if( !isPiecePinned(blackPcs[i]))
						{
							...
							if( K->x < fromX )
							{
								for( int x = K->x+1; x <= fromX; x++ )
								{
									if( blackPcs[i]->type == Pawn )
									{
										if( CanPawnReachSquare(blackPcs[i], x, K->y ))
										{
											mv.to.file = x;
											mv.to.rank = K->y;
											legalMoves->push_back(mv);
									...
									else if( CanPieceReachSquare(blackPcs[i], x, K->y ))
									{
										mv.to.file = x;
										mv.to.rank = K->y;
										legalMoves->push_back(mv);
					...
					if( !whitePcs[i]->bCaptured )
					{
						...
						if( !isPiecePinned(whitePcs[i] ))
						{
							...
							if( K->x < fromX )
							{
								if( K->y < fromY )
								{
									for( int x = K->x+1, y=K->y+1; (x<=fromX)&&(y<=fromY); x++, y++ )
									{
										if( whitePcs[i]->type == Pawn )
										{
											if( CanPawnReachSquare(whitePcs[i], x, y ))
											{
												mv.to.file = x;
												mv.to.rank = y;
												legalMoves->push_back(mv);
										...
										else if( CanPieceReachSquare(whitePcs[i], x, y ))
										{
											mv.to.file = x;
											mv.to.rank = y;
											legalMoves->push_back(mv);
							...
							if( K->x < fromX )
							{
								if( K->y < fromY )
								{
									for( int x = K->x+1, y=K->y+1; (x<=fromX)&&(y<=fromY); x++, y++ )
									{
										if( blackPcs[i]->type == Pawn )
										{
											if( CanPawnReachSquare(blackPcs[i], x, y ))
											{
												mv.to.file = x;
												mv.to.rank = y;
												legalMoves->push_back(mv);
										...
										else if( CanPieceReachSquare(blackPcs[i], x, y ))
										{
											mv.to.file = x;
											mv.to.rank = y;
											legalMoves->push_back(mv);
					...
					if( GetPiecesAttackingSquare(WHITE, fromX, fromY, &attackers ))
					{
						for( int i = 0; i < attackers.size(); i++ )
						{
							if( !attackers[i]->bCaptured )
								...
								{
									MyChessMove mv;
									mv.from.file = attackers[i]->x;
									mv.from.rank = attackers[i]->y;
									mv.to.file = fromX;
									mv.to.rank = fromY;
									legalMoves->push_back(mv);
									if( (attackers[i]->type == Pawn) && (mv.to.rank == 7) )
									{
						...
						for( int i = 0; i < attackers.size(); i++ )
						{
							if( !attackers[i]->bCaptured )
								if( !isPiecePinned(attackers[i]))
								{
									...
									mv.to.rank = fromY;
									legalMoves->push_back(mv);
									if( (attackers[i]->type == Pawn) && (mv.to.rank == 0) )
									{
		...
		if( P->color == WHITE )
		{
			if( P->y == toRank-1 )
			{
				...
				else if( (P->x == toFile-1)|| (P->x ==toFile+1))
				{
					if( toFile == this->empassantTarget.sq.file && 
						...
						this->empassantTarget.sq.rank == 5 && this->canEmpasse)
					{
						almost = true;
					}
			...
			else if( P->y == 1 )
			{
				if( P->y == toRank-2 )
				{
					if( P->x == toFile )
					{
						if( (board[toFile][toRank] == 0) && (board[toFile][toRank-1] == 0))
							almost = true;
					}
				}
			}
		...
		else 
		{
			if( P->y == toRank+1 )
			{
				...
				else if( (P->x == toFile-1)|| (P->x ==toFile+1) )
				{
					if( toFile == this->empassantTarget.sq.file && 
						toRank == this->empassantTarget.sq.rank &&
						this->empassantTarget.sq.rank == 2&& this->canEmpasse)
					{
						almost = true;
					}
					else if( board[toFile][toRank] != 0 )
		...
		case bRook:
			{
				...
				if( extraBlackRooks == false )
				{
					...
					if( R1->x == fromFile && R1->y == fromRank )
					{
						moverR1 = true;
					}
					else if( R2->x == fromFile && R2->y == fromRank )
					{
						moverR2 = true;
					}

					bool pinnedR1 = isPiecePinned(R1);
					bool pinnedR2 = isPiecePinned(R2);

					if( pinnedR1 && canReachR1)
					{
						if( (R1->x == toFile) && (R1->x == R1->PinFromX ))
						{
							//canReachR1 = true;
						}	
						else if( (R1->y == toRank) && (R1->y == R1->PinFromY ))
						{
							//canReachR1 = true;
						}
						else
					...
					if( pinnedR2 && canReachR2)
					{
						if( (R2->x == toFile) && (R2->x == R2->PinFromX ))
						{
							//canReachR2 = true;
						}	
						else if( (R2->y == toRank) && (R2->y == R2->PinFromY ))
						{
							//canReachR1 = true;
						}
						else
	...
	void makemove( int x1, int y1, int x2, int y2, bool promote, int type, bool isCastle = false )
	{
		...
		/*	if( pc && (pc->type == Pawn) )
			{
				if( (x2 == this->empassantTarget.sq.file) &&
					(y2 == this->empassantTarget.sq.rank))
				{
					if( mov == 2 || mov == 0 )
					{
						board[x2][y2-1]=0;
	...
	MyChessSquare findWhiteQueen(int x, int y)
	{
		...
		sq.file = (char)whitePcs[3]->x;
		sq.rank = (char)whitePcs[3]->y;
	...
	int GetParseError()
	{
		return parse_error_ParseMove;
	}

	int parse_error_ParseMove;
	...
	if( len == 3 )
	{
		...
		//MyChessPiece *rook2;
				...
				else
				{
					...
					if(this->isPiecePinned(r1) && movr1)
					{
	
							if( ((fil == r1->PinFromX) && (fil == r1->x)) ||
								((rank == r1->PinFromY) && (rank == r1->y)))
							{
								movr1 = true;
							}
							else movr1 = false;
					...
					if( isPiecePinned(r2)&& movr2)
					{
							if( ((fil == r2->PinFromX) && (fil == r2->x)) ||
								((rank == r2->PinFromY) && (rank == r2->y)))
							{
								movr2 = true;
							}
							else movr2 = false;
