...
/*	wxSizer *sizerPlusMinus = new wxBoxSizer(wxVERTICAL);
	...
	*/
	...
	sizerControls->Add(analyzeBtn,  wxALL);
	sizerControls->Add(stopBtn,  wxALL);
	sizerControls->Add(lockEngineToMove,  wxALL);
	sizerControls->Add(copyUp,  wxALL);
	sizerControls->Add(engineOptions,  wxALL);
	//sizerInfoText->Add(plusBtn, 0, wxEXPAND, 2);
	//sizerInfoText->Add(sizerPlusMinus, 0, wxALL, 2);	
	sizerControls->Add(plusBtn,  wxALL);
	sizerControls->Add(minusBtn,  wxALL);	
	//sizerInfoText->Add(m_textErr, 0, wxEXPAND , 2);
		sizerInfoText->Add(m_textNodes, 0,  wxALL );
	//sizerInfoText->Add(m_textCurrMove, 0, wxEXPAND );
	sizerInfoText->Add(m_textTime, 0,  wxALL );
	sizerInfoText->Add(m_textErr, 0,  wxALL );
	
	//sizerInfoText
	//	sizerControls
	sizerTop->Add(sizerInfoText, 0,  wxTOP| wxALL);
	sizerTop->Add(sizerControls, 0,  wxALL);
	...
	fa_goMode = GO_INFINITE;
	fa_goDepth = 10;
	fa_goTime = 20000;

	fa_goFixedTimeFinished = false;
	fa_goFixedDepthFinished = false;
...
//---------------------------------------------------------------------------
void MyPipePanel::SetGUIFont(wxFont &f2)
{
	wxFont font = f2;
	wxSize pSize = font.GetPixelSize();
...
//---------------------------------------------------------------------------
void MyPipePanel::SetPanelNameAndPersonalityIntoCaption(wxString &personality)
{
	wxString kbitzing_by = "Analysis Engine: ";


	kbitzing_by += panelName;
	kbitzing_by += " (" +personality + ")";
...
void MyPipePanel::SendMoves()
{
	...
	if( (pMovePtr->white == "") && /*(pMovePtr->x == 0)*/!pMovePtr->MoveFlagIsSet(MOVE_FLAG_X) && (pMovePtr->black == "") )
	{
		if( pParent->currentGame->gameBeginsAtStartPos == false)
		{
			SendFen();
	...
	/*if( pParent->currentGame->pCurrMove == pParent->currentGame->pBase )
	{
		multiPvLines.pMoveAnalyzed = pParent->currentGame->pRoot;
	}
	else*/
	{
	...
	if( mode == 2 )
	{
		...
		if( storedMovNum != 0 )
			multiPvLinesFA.SetCurrPly((storedMovNum-1)*2+white_black);

		MPVMoveNodeVectorArray *pMNVA = multiPvLinesFA.GetCurrent();
		if( pMNVA )
		{
			multiPvLinesFA.GetCurrent()->pMoveAnalyzed = pParent->currentGame->pCurrMove;
		}
	}
...
//---------------------------------------------------------------------------
void MyPipePanel::SetTimeMode(int TimeMode)
{
	//pParent->gameClock->m
	timeMode = TimeMode;
...
//---------------------------------------------------------------------------
void MyPipePanel::SendMPVModeOption(int numPVs)
{
	num_kibitz_lines = numPVs;
	string outPutString = "";
	outPutString = "setoption name MultiPV value ";

	char buff[20];
	_itoa(num_kibitz_lines, buff, 10);


	outPutString += buff;
	outPutString += "\n";

	(m_process)->GetOutputStream()->Write(outPutString.c_str(), outPutString.length());
}
...
//---------------------------------------------------------------------------
void MyPipePanel::ClearGameAnalysisBuffers()
{
	gameModeDepthPV.Clear();
	
	for( int i=0; i < num_kibitz_lines; i++ )
	{
		multiPvLines.ClearMPV(i);
	}

	multiPvLinesFA.Clear();
}
...
void MyPipePanel::WaitBestMove(char* buf, wxTextCtrl *text)
{
	...
	/*if( (pParent->currentGame->pCurrMove == pParent->currentGame->pRoot)&&
	...
	if( bestmove != "" )
	{
		...
		MyChessMove mvT;
		mvT.from.file = inptstr[0]-'a';
		mvT.from.rank = inptstr[1]-'0';
		mvT.to.file = inptstr[2]-'a';
		mvT.to.rank = inptstr[3]-'0';
		mvT.promo = -1;
		internalBoard.GenLegalMoves();
		for( int i = 0; i < internalBoard.numLegalMoves; i++ )
		{
			if( mvT == internalBoard.legal_moves[i].mv )
			{
				moveIsLegal = true;
				break;
			}
		...
		if( !moveIsLegal )
		{
			wxMessageBox(wxString::Format("%s Made an Illegal Move: %s", this->GetEngineName().c_str().AsChar(), bestmove.c_str()),"");
			pParent->m_engineVsEngine.numGamesPlayed--;
			this->DoFinishGame();
			return;
		}
		...
		//pParent->DoLookupFromBook();
		//pParent->moveDataCanvas->Refresh(true);
		...
		if( timeMode > HYPER_BULLET )
		{
			if( pParent->m_mgr.GetPane("scoreGraph").IsShown() )
			{
				...
				pParent->m_scoreGraphPanel->Refresh();
			}
			pParent->my_canvas->needsRefresh = true;
			pParent->my_canvas->Refresh();
			pParent->treeCanvas->MyRefresh(false);
		}
...
//	PGN::PgnLoad* pLoadPGN = pParent->pgnLoad;
	...
	if( !LockEngineToMove )
	{
		if( (pParent->currentGame->pCurrMove == pParent->currentGame->pRoot)&&
			(pParent->currentGame->gameBeginsAtStartPos == false)  )
		{
			int mvAt = atoi(&old_fen[old_fen.length()-1]);
			movNum = mvAt;
			storedMovNum = mvAt;
		}
		else
	...
	for( int i = 0; i < tokens.size(); i++ )
	{
		...
		if( tokens[i] == "depth" )
		{
			if( i+1 < tokens.size() )
			{
				...
				{
					if( mpvNum == 1 )
						multiPvLines.pMoveAnalyzed->depthWhite = atoi(tokens[i+1].c_str());
					
				}
				else
				{
					if( mpvNum == 1 )
						multiPvLines.pMoveAnalyzed->depthBlack = atoi(tokens[i+1].c_str());
...
//---------------------------------------------------------------------------
void MyPipePanel::DoFullAnalyze(char* buf, wxTextCtrl *text)
{
	//ChessBoard *board = pParent->board_canvas->GetBoard();
//	PGN::PgnLoad* pLoadPGN = pParent->pgnLoad;
	string old_fen = internalBoard.Fen();
...
/*	if (logToFile == true)
	...
	int addedLine = 0;
	bool mpvToggle = false;
	multiPvLinesFA.PvChangedThisFrame(false);
...
//	int numDepths = 0;
	for( int i = 0; i < tokens.size(); i++ )
	{
		...
		if( tokens[i] == "depth" )
		{
			if( i+1 < tokens.size() )
			{
				...
				if( depthV > maxDepthV )
				{
					...
					if( this->fa_goMode == GO_FIXED_DEPTH )
					{
						if( depthV > this->fixedDepth )
						{
							this->fa_goFixedDepthFinished = true;
							//break;
						}
					}

					if( this->fa_goMode == GO_FIXED_DEPTH_AND_TIME )
					{
						if( depthV > this->fixedDepth )
						{
							this->fa_goFixedDepthFinished = true;
					...
					else if( this->fa_goMode == GO_FIXED_DEPTH_OR_TIME )
					{
						if( depthV > this->fixedDepth )
						{
							this->fa_goFixedDepthFinished = true;
				...
				if( white_black == BLACK )
				{
					if( mpvNum == 1 )
					{
						...
						if( pMNVA )
						{
							multiPvLinesFA.GetCurrent()->pMoveAnalyzed->depthWhite = (unsigned char)atoi(tokens[i+1].c_str());
						}
					}
				}
				else
				{
					if( mpvNum == 1 )
					{
						...
						if( pMNVA )
						{
							multiPvLinesFA.GetCurrent()->pMoveAnalyzed->depthBlack = (unsigned char)atoi(tokens[i+1].c_str());
						}
					}
				}
				...
				if( mpvNum == 0 )
				{
					...
					if( pMNVA )
					{
						multiPvLinesFA.GetCurrent()->mpvs[0].SetDepthString(tokens[i+1]);// = AddNode(VarMoveNode(pgn, *last_move, fen, white_black, string(inf)),0);
					}	
				}
				else
				{
					//mpvStr[mpvNum-1] += out;
					//multiPvLinesFA.AddNode(VarMoveNode(pgn, *last_move, fen, white_black, string(inf)),mpvNum-1);
					MPVMoveNodeVectorArray *pMNVA = multiPvLinesFA.GetCurrent();
					if( pMNVA )					
					{
						multiPvLinesFA.GetCurrent()->mpvs[mpvNum-1].SetDepthString(tokens[i+1]);
					}		
				}
			}
			//i++;
		...
		if( tokens[i] == "multipv" )
		{
			if( i+1 < tokens.size() )
			{
				...
				if( tc < MAX_LINES )
				{
					...
					if( pMNVA )
					{
						multiPvLinesFA.GetCurrent()->ClearMPV(mpvNum -1);
					}
		...
		if( tokens[i] == "bestmove" )
		{
			...
			if( this->fa_goMode == GO_FIXED_TIME )
			{
				if( !this->pParent->m_fullAnalysisSettings.guiControlsTime )
				{
					fa_goFixedTimeFinished = true;
				}
			}
			else if( this->fa_goMode == GO_FIXED_DEPTH )
			{
				fa_goFixedDepthFinished = true;
		...
		if( tokens[i] == "mate" )
		{
			...
			if( i+1 < tokens.size() )
			{
				...
				if( white_black == BLACK )
				{
					...
					if( mpvNum == 1 )
					{
						...
						if( pMNVA )
						{
							multiPvLinesFA.GetCurrent()->pMoveAnalyzed->move_cp_white = -int(num);
						}
					}
				}
				else
				{
					...
					if( mpvNum == 1 )
					{
						...
						if( pMNVA )
						{
							multiPvLinesFA.GetCurrent()->pMoveAnalyzed->move_cp_black = int(num);
						}
					}
				}
				float score_cp = num;
			...
			//	sprintf(floatBuff, "0.00");
				...
				float fnum = (float)num;
				if( mpvNum == 0 && num_kibitz_lines == 1) 
					multiPvLinesFA.SetScore(score_cpi,fnum , fltBuff, 0);
				else if( mpvNum != 0 && mpvToggle == true)
					multiPvLinesFA.SetScore(score_cpi, fnum, fltBuff, mpvNum-1);
			}
			else break;
		}
		...
		if( tokens[i] == "cp" )
		{
			if( i+1 < tokens.size() )
			{
				...
				/*	if( mpvNum == 0 )
					...
					int score_cpi = (int) score_cp;
					if( mpvNum == 0 && num_kibitz_lines == 1) 
						multiPvLinesFA.SetScore(score_cpi, num, fltBuff, 0);
					else if( mpvNum != 0 && mpvToggle == true)
						multiPvLinesFA.SetScore(score_cpi, num, fltBuff, mpvNum-1);
				}
			}
		}
		...
		if( tokens[i] == "pv" )
		{
			...
			multiPvLinesFA.PvChangedThisFrame(true);
			if( num_kibitz_lines != 1 )
			{
				if( mpvNum == 0 )
					break;
			...
			while(j < tokens.size())
			{
					...
					(tokens[j][tokenjl-1] == 'b') )))
				{
					if( tokens[j]!= "info" && tokens[j] != "inf")
					{
						...
						{
							mpvStr[0] += out;
							multiPvLinesFA.AddNode(VarMoveNode(pgn, *last_move, fen, white_black, inf),0);
							
						}
						else
						{
							mpvStr[mpvNum-1] += out;
							multiPvLinesFA.AddNode(VarMoveNode(pgn, *last_move, fen, white_black, inf),mpvNum-1);
...
/*	if( addedLine != 0)
	...
	if(this->fa_goMode == GO_FIXED_DEPTH)
	{
		if( (maxDepthV == 1) || (mpvNum < num_kibitz_lines) )
		{
			this->fa_goFixedDepthFinished = true;
		}
	}
/*	if(this->fa_goMode == GO_FIXED_DEPTH)
	...
	if(this->fa_goMode == GO_FIXED_DEPTH_OR_TIME)
	{
		if( (maxDepthV == 1) || (mpvNum < num_kibitz_lines) )
		{
			this->fa_goFixedDepthFinished = true;
	...
	if( addedLine != 0)
	{
		...
		for( int i = 0; i < num_kibitz_lines; i++ )
		{
			text->AppendText(char('0'+i));
			text->AppendText(" (");
			text->AppendText(multiPvLinesFA.mpvs[i].score_str);
			text->AppendText(" ) ");
			for( int j = 0; j < multiPvLinesFA.mpvs[i].moves.size(); j++ )
			{
				if( j==0 && !multiPvLinesFA.mpvs[i].moves[j].whitesMove )
				{
					text->AppendText( multiPvLinesFA.mpvs[i].moves[j].info);
					text->AppendText("...");
				}
				if( multiPvLinesFA.mpvs[i].moves[j].whitesMove )
				{
					text->AppendText( multiPvLinesFA.mpvs[i].moves[j].info);
					text->AppendText(".");
				}
				text->AppendText( multiPvLinesFA.mpvs[i].moves[j].pgnMove);
				text->AppendText(" ");
			}
			text->AppendText("\n");
		}
...
//	wxSleep(1);
	...
	{
		if( this->fa_goMode == GO_INFINITE )
		{
			SendGoInfinite();
		}
		else if( this->fa_goMode == GO_FIXED_TIME )
		{
		...
		else if( this->fa_goMode == GO_FIXED_DEPTH_AND_TIME )
		{
			SendGoInfinite();
		}
		else if( this->fa_goMode == GO_FIXED_DEPTH_OR_TIME )
...
//---------------------------------------------------------------------------
void MyPipePanel::CopyAllToNotation()
	{
		if( this->startedAnalyzing == true )
		{
			this->KibitzStartsMove();
		}
		PGN::Move *pMove = pParent->currentGame->pCurrMove;
		this->CopyAllToGameTree();
		pParent->currentGame->pCurrMove = pMove;
	}
...
void MyPipePanel::CopyLineToGameTree(int mpv)
{
	...
	string oldFen = internalBoard.Fen();
	...
	for( int i = 0; i< multiPvLines.mpvs[mpv].moves.size(); i++)
	{
		...
		if( white_black == 0 )
		{
			internalBoard.SetBoard(pMovePtr->m_fenBlack);
		}
		else
		{
			internalBoard.SetBoard(pMovePtr->m_fenWhite);
		}
		internalBoard.GenLegalMoves();
		bool bLegal = false;
		for( int j = 0; j < internalBoard.numLegalMoves; j++ )
		{
			if( internalBoard.legal_moves[j].mv == multiPvLines.mpvs[mpv].moves[i].move )
			{
				bLegal = true;
				break;
			}
		...
		if( i == 0 )
		{
			...
			pMovePtr->Visit();// = true;
			char buff[100];
			sprintf(buff, "[%s, D=%d]",multiPvLines.mpvs[mpv].score_str.c_str(), multiPvLines.mpvs[mpv].depth);
			//multiPvLines.mpvs[mpv].depth
			pMovePtr->WriteScoresToAnnotation(string(buff), !white_black);
		}
		else
		{
			PGN::MoveInserter mv_inserter(  pgn_string,  white_black,  fen, &epn);
...
//---------------------------------------------------------------------------
void MyPipePanel::CopyLineToGameTreeFA(int mpv)
{
	...
	PGN::Move *pMovePtr = multiPvLinesFA.GetCurrent()->pMoveAnalyzed;// pParent->currentGame->pCurrMove;
	for( int i = 0; i< multiPvLinesFA.GetCurrent()->mpvs[mpv].moves.size(); i++)
	{
		string fen = multiPvLinesFA.GetCurrent()->mpvs[mpv].moves[i].fen;
		MyChessMove epn = multiPvLinesFA.GetCurrent()->mpvs[mpv].moves[i].move;
		int white_black = (int)!multiPvLinesFA.GetCurrent()->mpvs[mpv].moves[i].whitesMove;
		string pgn_string = multiPvLinesFA.GetCurrent()->mpvs[mpv].moves[i].pgnMove;
		if( white_black == 0 )
		{
			internalBoard.SetBoard(pMovePtr->m_fenBlack);
		}
		...
		for( int j = 0; j < internalBoard.numLegalMoves; j++ )
		{
			if( internalBoard.legal_moves[j].mv == multiPvLinesFA.GetCurrent()->mpvs[mpv].moves[i].move )
			{
				bLegal = true;
				break;
			}
		...
		if( i == 0 )
		{
			...
			if( pParent->m_fullAnalysisSettings.writeScores )
			{
				char buff[100];
				sprintf(buff, "[%s, D=%s]",multiPvLinesFA.GetCurrent()->mpvs[mpv].score_str.c_str(), multiPvLinesFA.GetCurrent()->mpvs[mpv].depthStr.c_str());
				pMovePtr->WriteScoresToAnnotation(/*multiPvLinesFA.GetCurrent()->mpvs[mpv].score_str*/string(buff), !white_black);
			}
		}
		else
		{
...
void MyPipePanel::OnBtnEngineOptions(wxCommandEvent& WXUNUSED(event))
{
	...
	KibitzStartsMove();
	EngineOptionsModalDialog dlg(this, this, this->engineOptions, wxID_ANY,"Engine Options");
